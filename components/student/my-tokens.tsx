...
function QRCodeDisplay({ tokenId }: { tokenId: string }) {
  const size = 180
...
export function MyTokens() {
  ...
  const loadTokens = async () => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`/api/tokens?studentId=${user.studentId}`, { cache: 'no-store' })
              ...
              <Card key={token.id} className="border-border/60 overflow-hidden">
                ...
                <CardContent className="space-y-4">
                  ...
                  >
                    ...
                    <QRCodeDisplay tokenId={token.id} />
                    {token.backupCode && (
                      <div className="mt-3 text-center">
                        <p className="text-[11px] text-muted-foreground uppercase tracking-wide">Backup Code</p>
                        <p className="font-mono text-sm font-semibold text-foreground">{token.backupCode}</p>
                      </div>
                    )}
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    {token.backupCode && (
                      <p className="text-xs font-mono text-muted-foreground">Backup: {token.backupCode}</p>
                    )}
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          {expandedToken && (
            <div className="p-4">
              ...
              <QRCodeDisplay tokenId={expandedToken} />
              {(() => {
